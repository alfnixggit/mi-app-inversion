import streamlit as st
import pandas as pd
from datetime import datetime
import gspread
from google.oauth2.service_account import Credentials

# --- LA LLAVE (Pegada tal cual la pasaste) ---
INFO_LLAVE = {
  "type": "service_account",
  "project_id": "careful-broker-395321",
  "private_key_id": "75c0576acd9256ef5c04419f9e4a6df4d7a8c2e8",
  "private_key": "-----BEGIN PRIVATE KEY-----\\\\n-----END PRIVATE KEY-----\n",
  "client_email": "python-app@careful-broker-395321.iam.gserviceaccount.com",
  "client_id": "108343647994793129429",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/python-app%40careful-broker-395321.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}

st.set_page_config(page_title="InversionData Direct", page_icon="💰")
st.title("💰 Carga Directa (Intento 2)")

hoy = datetime.now().strftime('%d/%m/%Y')

# --- CONEXIÓN MEJORADA ---
@st.cache_resource
def conectar():
    # TRUCO: Asegurar que los saltos de línea sean reales
    key = INFO_LLAVE["private_key"].replace("\\n", "\n")
    if not key.startswith("-----BEGIN"):
        key = "-----BEGIN PRIVATE KEY-----\n" + key
    if not key.endswith("-----END PRIVATE KEY-----\n") and not key.endswith("-----END PRIVATE KEY-----"):
        key = key + "\n-----END PRIVATE KEY-----"
    
    INFO_LLAVE["private_key"] = key
    
    scopes = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']
    creds = Credentials.from_service_account_info(INFO_LLAVE, scopes=scopes)
    return gspread.authorize(creds)

try:
    gc = conectar()
    sh = gc.open("Inversiondata")
    
    # Cargar Activos
    ws_activos = sh.worksheet("APP1")
    activos = ws_activos.col_values(1)
    
    # Cargar Histórico para proponer valores
    ws_log = sh.worksheet("LOG_MOV_PYTHON")
    data = ws_log.get_all_values()
    df_log = pd.DataFrame(data[1:], columns=data[0]) if len(data) > 1 else pd.DataFrame(columns=["FECHA", "ACTIVO", "VALOR"])

    # --- DIBUJAR FORMULARIO ---
    datos_finales = {}
    for activo in activos:
        # Buscar el valor de la última carga para proponerlo
        ultimo_valor = "0"
        if not df_log.empty:
            filtro = df_log[df_log["ACTIVO"] == activo]
            if not filtro.empty:
                ultimo_valor = filtro.iloc[-1]["VALOR"]
        
        # Semáforo de hoy
        ya_hecho = any((df_log["ACTIVO"] == activo) & (df_log["FECHA"] == hoy))
        
        c1, c2, c3 = st.columns([0.1, 0.5, 0.4])
        with c1: st.write("✅" if ya_hecho else "🔴")
        with c2: st.write(f"**{activo}**")
        with c3:
            # Proponemos el último valor guardado
            val = st.text_input("Importe", value=ultimo_valor, key=activo, label_visibility="collapsed")
            datos_finales[activo] = val

    st.divider()
    if st.button("💾 GUARDAR JORNADA COMPLETA", use_container_width=True):
        filas_nuevas = [[hoy, a, v] for a, v in datos_finales.items()]
        ws_log.append_rows(filas_nuevas)
        st.success("¡Datos volcados al Excel con éxito!")
        st.cache_resource.clear()
        st.rerun()

except Exception as e:
    st.error(f"Error en Intento 2: {e}")
